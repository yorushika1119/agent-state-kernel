...
async def test_dispatch_records_user_message_as_task_conversation_ref():
    ...
    try:
        ...
        assert [ref.message_ref_id for ref in refs] == ["msg_second", "msg_first"]
        assert refs[0].text_summary == "这个任务现在进度怎么样"
        assert observer_view["recent_conversation_refs"][0]["message_ref_id"] == "msg_second"
        assert manager_view["recent_conversation_refs"][0]["message_ref_id"] == "msg_second"
    finally:
        await store.close()
