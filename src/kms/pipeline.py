...
async def reduce(
    ...
    current_intent = await store.get_intent(session_id)
    ...
    current_plan = await store.get_plan(session_id)
    ...
    commitments = await store.get_commitments(session_id)
...
async def refresh_progress(store, session_id: str) -> ProgressState:
    ...
    beliefs = await store.get_beliefs(session_id)
    intent = await store.get_intent(session_id)
...
async def run_pipeline(
    ...
    if _should_arbitrate(primary_event.event_type):
        ...
        existing_beliefs = await store.get_beliefs(session_id)
